/*	Genetic Algorithm	start from 2018.2.10   */
#include<iostream>
#include<ctime>
#include<random>
#include<array>
...
int compare(const void *, const void *);//   ???  ¡ú¡ú¡ú"NEED MORE RESEARCH!!!¡û¡û¡û  ???
...
int main()
{
	...
	//¢Ùinitialize the population ¢Úcalculate fitness
	//¢Ûadd fitness ¢Ücaculate fitness proportion ¢Ýinput fitness proportion
	for (size_t pop = 0; pop < popScale; pop++)
	{
		initialize(x1[Gen][pop], x2[Gen][pop], x3[Gen][pop], x4[Gen][pop]);//¢Ù
		fitness[pop] = evaluateFitness(x1[Gen][pop], x2[Gen][pop], x3[Gen][pop], x4[Gen][pop]);//¢Ú
	}
	totalFitness = getTotalFitness(fitness);//¢Û
	for (size_t i = 0; i < popScale; i++)
	{
		sortedFitness[i].data = fitness[i] / totalFitness;//¢Ü¢Ý
	...
	//EVOLUTION
	while (1 - bestIndividual > precision)
	{
